...
2:stderr
 ...
 < redirect stdin from keyboard to file, open file, apply command to text in file 
			...
			fd = open(filename)
			dup2(fd, 0) //duplicate the stdin filedescriptor
...
main(void)
{
	...
	while(fgets(buf, MAXLINE, stdin) != NULL){
		...
		/*if the fork was sucessful run the child process*/
			...
			for (i =0; i<count; i++){
				while (token != NULL){
					/*push data in the the options array*/
			...
			for(int i=0; i<count; i++){
				if (options[i%count]=="<"){
					...
					int fdIN = fopen(&fName); 
					dup2(fdIN, 0); //duplicate the stdin file descriptor
					close(fdIN); 
				}
					...
					dup2(fdOUT, 1); //duplicate the stdin file descriptor	
					close(fdOut); 
				}
				else if (options[i%count] == ">"){
					char *fName; 
					...
					dup2(fdOUT, 1); //duplicate the stdout file descriptor
					dup2(fdERR, 2); //duplicate the stderr file descriptor
					close(fdOUT); 
					close(fdERR); 
